...
function node_create(coords, id, data){
    ...
    // Get node items
    grp_items = {}
    grp_fields = ['GIVEN NAME', 'SUR NAME', 'LOCATION', 'DATES']
...
//~             recentre += grid * 2
        ...
        if(data['NAME']['SURN'] != null){
            ...
            if(grp_items['SUR NAME'].lines.length < 2){
                 grp_item_translate(['SUR NAME'], grid * -1)
                 grp_item_translate(['DATES', 'LOCATION'], grid * 1)
//~                 recentre += grid * 1
            }            
        } else{
    ...
    // DATES
    grp_items['DATES'].contents = ''
    var has_birthdate = true
    var has_deathdate = true
 
    // BIRTH DATE
    if(data['BIRT'] != null && data['BIRT']['YEAR'] != null){

            grp_items['DATES'].contents = 'B: ' + data['BIRT']['YEAR']
...
//~         recentre += grid * 2
    ...
    if(data['DEAT'] != null && data['DEAT']['YEAR'] != null){  
        ...
        // Append to dates
        if(grp_items['DATES'].contents == ''){
            grp_items['DATES'].contents = 'D: ' + data['DEAT']['YEAR']
        }else{
            grp_items['DATES'].contents = grp_items['DATES'].contents + ' - D: ' + data['DEAT']['YEAR']
        }
//~     }else{
...
//*           recentre -= grid * 2
    ...
    // Remove DATES if none are available
    if(!has_deathdate & !has_birthdate){
        grp_item_remove(['DATES'])
    }
    
    // LOCATION (birth place)
    if(data['BIRT'] != null && data['BIRT']['PLAC'] != null){
        grp_items['LOCATION'].contents = data['BIRT']['PLAC']
    }else{
        grp_item_remove(['LOCATION']) 
    }
...
function draw_links(fam_id){
    ...
    if(fam.wide_spouse != null ){alert('WIDE')}
    
    // Adjust offsets for 'wide' spouses links
    this_link_vert_offset = link_vert_offset
    this_link_lat_offset = link_lat_offset
    if(fam.wide){ 
        this_link_vert_offset += grid
        this_link_lat_offset -= grid
    }
        ...
        wife_centr = node_centre_get(fam.WIFE)
         ...
         if(husb_centr != null & wife_centr != null){
            ...
            if(husb_centr[0] > wife_centr[0]){
                husb_centr[0] = husb_centr[0] - this_link_lat_offset
                wife_centr[0] = wife_centr[0]+this_link_lat_offset
            }else{
                husb_centr[0] = husb_centr[0] + this_link_lat_offset
                wife_centr[0] = wife_centr[0] - this_link_lat_offset
            }
            ...
            link_create([ 
                husb_centr,
                [husb_centr[0], husb_centr[1] - this_link_vert_offset],
                [wife_centr[0], wife_centr[1] - this_link_vert_offset],
                wife_centr
            ],  fam_grp, link_style)            
            ...
            if(fam.CHIL != null){
                ...
                for(var i = 0; i < fam.CHIL.length; i ++){
                    ...
                    if(chil_centr != null){
                        ...
                        // Child link
                        link_create([ 
                            [chil_centr[0], chil_centr[1] + this_link_vert_offset],
                            chil_centr
                        ],  fam_grp, link_style)   
                    }
                }
                 ...
                 if(chil_lines_x.length > 0){
					...
					chil_lines_y.sort(function(a, b){return a-b})
                      ...
                      chil_lines_y_mid = chil_lines_y[0]
                       ...
                       link_create([ 
							[chil_lines_x_min, chil_lines_y_mid + this_link_vert_offset],
							[chil_lines_x_max, chil_lines_y_mid + this_link_vert_offset]
                        ],  fam_grp, link_style)   
                        ...
                        if(mid_pt >= chil_lines_x_min & mid_pt <= chil_lines_x_max){
                            chilspouse_coords = [
                                [mid_pt, husb_centr[1] - this_link_vert_offset],
                                [mid_pt, chil_lines_y_mid + this_link_vert_offset]
                            ]
